...
func main() {
	listenAddr := flag.String("a", "127.0.0.1:8443", "Listen address")
	privateKey := flag.String("k", "", "Private key file path (autogenerated if not specified)")
	showPrivateKey := flag.Bool("showprivatekey", false, "Write the autogenerated private key PEM to stderr (if autogenerated)")
	certificateChain := flag.String("c", "", "Certificate chain file path (autogenerated if not specified)\nSee https://pkg.go.dev/crypto/tls#LoadX509KeyPair for details")
	getFromStdin := flag.Bool("i", false, "Get the private key and certificate chain from stdin")
